...
vignette: >
  %\VignetteIndexEntry{LAPOP R Labels Guide}
...
data_path <- rprojroot::find_package_root_file("C:/Users/rob/Box/LAPOP Shared/2_Projects/2023 AB/BRA/Data Processing")
```
...
data1 <- readstata13::read.dta13(paste0(data_path, "/BRA 2023 LAPOP AmericasBarometer (v1.0s).dta"), 
                                 generate.factors = T)
names(attributes(data1))

# HAVEN PACKAGE
data2 <- haven::read_dta(paste0(data_path, "/BRA 2023 LAPOP AmericasBarometer (v1.0s).dta"))
names(attributes(data2))

# RIO PACKAGE
data3 <- rio::import(paste0(data_path, "/BRA 2023 LAPOP AmericasBarometer (v1.0s).dta"))
...
Both approaches extract the third element (x[3]) from each list in the "expansion.fields" attribute of data1, but only if the first element (x[1]) equals "ing4" (i.e., the variable of interest). Both produce a cleaned list of matching third nested elements.
