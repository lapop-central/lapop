...
# READSTATA13 PACKAGE (RECOMMENDED) TO OBTAIN METADATA INSIDE expasion.fields
data1 <- readstata13::read.dta13("<AB-DATA-PATH>", generate.factors = T)
names(attributes(data1))

# HAVEN PACKAGE
data2 <- haven::read_dta("<AB-DATA-PATH>")
names(attributes(data2))

# RIO PACKAGE
data3 <- rio::import("<AB-DATA-PATH>")
...
# Let's use the AmericasBarometer 2023 Brazil dataset. 
data(cm23) 

# Extracting attributes from expasion.fields
varlabels = attr(cm23, "expansion.fields")
...
# English
cm23<-lpr_set_attr(cm23, notes, noteid = "note4", attribute_name = "qwording_en") 

# Spanish
cm23<-lpr_set_attr(cm23, notes, noteid = "note7", attribute_name = "qwording_es") 

# Portuguese
cm23<-lpr_set_attr(cm23, notes, noteid = "note1", attribute_name = "qwording_pt") 

# Printing 
attr(cm23$ing4, "qwording_es")
attr(cm23$ing4, "qwording_en")
attr(cm23$ing4, "qwording_pt"); 
```
...
# Base R approach
Filter(Negate(is.null), lapply(attr(cm23, "expansion.fields"), 
                               function(x) if (x[1] %in% c("ing4")) x[3]))

# purrr package approach
purrr::compact(purrr::map(attr(cm23, "expansion.fields"), ~ 
...
```{r, ROs, eval=FALSE}
cm23 <- lpr_set_ros(cm23) # Default English
cm23 <- lpr_set_ros(cm23, lang_id = "es", attribute_name = "respuestas") # Spanish
cm23 <- lpr_set_ros(cm23, lang_id = "pt", attribute_name = "ROsLabels_pt") # Portuguese

attr(cm23$ing4, "roslabel"); attr(cm23$ing4, "respuestas"); attr(cm23$ing4, "ROsLabels_pt")
```
